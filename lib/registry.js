...
Registry.prototype.configureModel = function(ModelCtor, config) {
  ...
  if (typeof config.methods === 'object' && config.methods !== null) {
    settings.methods = config.methods;
  }
  ...
  var baseName = ModelCtor.base.modelName || 'PersistedModel';
  BaseModel = this.getModel(baseName);
  var model = BaseModel.extend(modelName, {}, settings);

  // Remote methods
  this._defineRemoteMethods(ModelCtor, model.settings.methods);
};
